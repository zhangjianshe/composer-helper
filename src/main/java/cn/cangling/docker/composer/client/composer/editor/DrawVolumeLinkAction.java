...
public class DrawVolumeLinkAction implements IMouseAction {
    ...
    public IMouseAction onDown(MouseEvent event) {
        ...
        assert hitTestResult.getHitTest() == HitTest.HT_START_VOLUME;
    ...
    public void onUp(MouseEvent event) {
        ...
        if (hitTestResult.getHitTest() == HitTest.HT_OBJECT) {
            if (hitTestResult.getData() instanceof VolumeObject) {
                target = hitTestResult.getData();
                if(target instanceof VolumeObject)
                {
                    VolumeObject volumeObject = (VolumeObject) target;
                    //添加一个链接
                    ...
                    if (serviceObject instanceof ServiceObject) {
                       ...
                       boolean b= serviceObject1.linkToVolume(volumeObject.getRefObject());
                       if(b) {
